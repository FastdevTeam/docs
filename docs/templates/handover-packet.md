...
1. Executive context (problem, outcomes, success metrics)
2. Scope (MVP in/out, phase 2+ ideas)
3. Workflow model (story map/use-case slices + key exceptions)
4. Backlog (prioritized)
5. Integrations & data (map + owners + access plan)
6. NFR scenarios (prioritized, MVP-critical marked)
7. Security/procurement status (evidence pack readiness + blockers)
8. Decision log + open questions (owner + date)
